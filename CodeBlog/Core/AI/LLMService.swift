...
final class LLMService: LLMServicing {
    ...
    private static let timelineFailureToastThrottleQueue = DispatchQueue(
        label: "com.codeblog.timelineFailureToastThrottle"
    )
    private enum CodeBlogBackendConfig {
        static let defaultEndpoint = "https://web-production-f3361.up.railway.app"
        static let infoPlistEndpointKey = "CodeBlogBackendURL"
        static let userDefaultsEndpointOverrideKey = "codeblogBackendURLOverride"
    }
    ...
    private func resolvedCodeBlogEndpoint(savedEndpoint: String?) -> String {
        let defaults = UserDefaults.standard

        if let override = defaults.string(forKey: CodeBlogBackendConfig.userDefaultsEndpointOverrideKey)?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !override.isEmpty {
            return override
        }

        if let infoEndpoint = Bundle.main.infoDictionary?[CodeBlogBackendConfig.infoPlistEndpointKey] as? String {
        ...
        return CodeBlogBackendConfig.defaultEndpoint
    }
    ...
    private func makeBatchProvider(
        ...
        case .codeblog:
            ...
            if case .codeblogBackend(let endpointFromSettings) = providerType {
                savedEndpoint = endpointFromSettings
            } else {
                savedEndpoint = nil
            }
            let endpoint = resolvedCodeBlogEndpoint(savedEndpoint: savedEndpoint)
            guard let provider = makeCodeBlogProvider(endpoint: endpoint) else { throw noProviderError() }
            return (actions: BatchProviderActions(
    ...
    private func configuredBackupProvider(primaryProviderID: LLMProviderID) -> ConfiguredBackupProvider? {
        ...
        // CodeBlog backend isn't currently supported in provider settings.
        if backupProvider == .codeblog {
            return nil
        }
    ...
    private func buildFailureToastMessage(
        ...
        if rateLimited && !backupConfigured {
            return "CodeBlog hit a rate limit and no backup provider is configured. Add a backup in Settings > Providers to avoid interruptions."
        }

        switch operation {
        case .transcribing:
            return "CodeBlog couldn't transcribe this batch. Check Settings > Providers and configure a backup provider."
        case .generatingCards:
            return "CodeBlog couldn't generate timeline cards for this batch. Check Settings > Providers and configure a backup provider."
        case .none:
            return "CodeBlog couldn't finish this batch. Check Settings > Providers and configure a backup provider."
        }
    }
    ...
    private func makeTextProvider() throws -> TextProviderActions {
        ...
        case .codeblogBackend(let endpoint):
            let resolvedEndpoint = resolvedCodeBlogEndpoint(savedEndpoint: endpoint)
            guard let provider = makeCodeBlogProvider(endpoint: resolvedEndpoint) else { throw noProviderError() }
            return TextProviderActions(
                generateText: provider.generateText,
                generateTextStreaming: nil
            )
