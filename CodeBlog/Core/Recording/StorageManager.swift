...
protocol StorageManaging: Sendable {
    ...
    func deleteTimelineCard(recordId: Int64)   // Soft-delete a single card by its row id
    func debugDeleteAgentCards()               // DEBUG: soft-delete all Agent category cards
...
struct TimelineCard: Codable, Sendable, Identifiable {
    ...
    init(
        ...
        self.agentCardType = agentCardType
...
final class StorageManager: StorageManaging, @unchecked Sendable {
    ...
    private let dbWriteQueue = DispatchQueue(label: "com.codeblog.storage.writes", qos: .utility)
    ...
    func updateTimelineCardVideoURL(cardId: Int64, videoSummaryURL: String) {
        try? timedWrite("updateTimelineCardVideoURL") { db in
            try db.execute(sql: """
    ...
    /// Save an Agent-generated timeline card directly (no batch required).
    /// - Parameters:
    ///   - startDate: The real start time of the coding session this card covers.
    ///   - endDate:   The real end time.
    ///   - title:     Card title generated by AI.
    ///   - summary:   One-line summary.
    ///   - detailedSummary: Full content (used as post body for "post" type).
    ///   - agentCardType: "journal" | "insight" | "post"
    ///   - previewId: MCP preview_post ID, used later for confirm_post.
    @discardableResult
    func saveAgentTimelineCard(
        startDate: Date,
        endDate: Date,
        title: String,
        summary: String,
        ...
        let startTimestamp = timeFormatter.string(from: startDate)
        let endTimestamp   = timeFormatter.string(from: endDate)
        let startTs = Int(startDate.timeIntervalSince1970)
        let endTs   = Int(endDate.timeIntervalSince1970)
        let (dayString, _, _) = startDate.getDayInfoFor4AMBoundary()
        ...
        let metadataString = (try? encoder.encode(meta)).flatMap { String(data: $0, encoding: .utf8) }

        var lastId: Int64? = nil
        try? timedWrite("saveAgentTimelineCard") { db in
            try db.execute(sql: """
                INSERT INTO timeline_cards(
                    batch_id, start, end, start_ts, end_ts, day, title,
                    summary, category, subcategory, detailed_summary, metadata
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, arguments: [
                nil, startTimestamp, endTimestamp, startTs, endTs, dayString, title,
                summary, "Agent", agentCardType, detailedSummary, metadataString
            ])
            lastId = db.lastInsertedRowID
        }
        return lastId
    ...
    func deleteTimelineCard(recordId: Int64) {
        try? timedWrite("deleteTimelineCard") { db in
            try db.execute(sql: """
                UPDATE timeline_cards SET is_deleted = 1 WHERE id = ?
            """, arguments: [recordId])
        }
    }

    func debugDeleteAgentCards() {
        try? timedWrite("debugDeleteAgentCards") { db in
            try db.execute(sql: """
                UPDATE timeline_cards SET is_deleted = 1 WHERE category = 'Agent'
            """)
        }
    }
    ...
    func fetchUnreviewedTimelineCardCount(forDay day: String, coverageThreshold: Double = 0.8) -> Int {
        ...
        let cardFetch: (cards: [(start: Int, end: Int)], invalidCount: Int)?
        do {
            cardFetch = try timedRead("fetchUnreviewedTimelineCardCount.cards") { db -> (cards: [(start: Int, end: Int)], invalidCount: Int) in
                var invalidCount = 0
                ...
                let cards = rows.compactMap { row -> (start: Int, end: Int)? in
                    ...
                    if cat.caseInsensitiveCompare("System") == .orderedSame {
                        return nil
                    }
            ...
            print("[StorageManager] fetchUnreviewedTimelineCardCount DB error: \(error)")
            return 0
        }
    ...
    /// Creates a dummy "Installed CodeBlog!" card when onboarding completes.
    ...
    private func buildOnboardingSummary() -> String {
        ...
        case "gemini":
            return "You successfully installed CodeBlog and configured it with Gemini AI. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"

        case "chatgpt_claude":
            // Check which CLI tool they picked
            let cliTool = UserDefaults.standard.string(forKey: "chatCLIPreferredTool") ?? "claude"
            if cliTool == "codex" {
                return "You successfully installed CodeBlog with ChatGPT. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            } else {
                return "You successfully installed CodeBlog with Claude. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            }
        ...
        case "ollama":
            ...
            if localEngine == "lmstudio" {
                return "You successfully installed CodeBlog with LM Studio — your data stays 100% on your device. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            } else {
                return "You successfully installed CodeBlog with Ollama — your data stays 100% on your device. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
            }

        default:
            return "You successfully installed CodeBlog. Come back in 30 minutes to see your first real activity card! ✨ (This is a sample card, so you can see what your timeline will look like.)"
        }
    }
    ...
    private let purgeQ = DispatchQueue(label: "com.codeblog.storage.purge", qos: .background)
...
private extension StorageManager {
    func migrateLegacyChunkPathsIfNeeded() {
        ...
        let legacyBase = fileMgr.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Containers/\(bundleID)/Data/Library/Application Support/CodeBlog", isDirectory: true)
        let newBase = appSupport.appendingPathComponent("CodeBlog", isDirectory: true)
