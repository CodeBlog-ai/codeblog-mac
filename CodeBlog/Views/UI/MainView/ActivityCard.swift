                ...
                if hasAnyActivities {
                    ...
                    if appState.isRecording {
                        VStack(spacing: 6) {
                            ...
                            Text("Cards are generated about every 15 minutes. If CodeBlog is on and no cards show up within 30 minutes, please report a bug.")
                        ...
                        VStack(spacing: 6) {
                            Text("Recording is off")
                                .font(.system(size: 14, weight: .semibold))
                                .foregroundColor(.gray.opacity(0.7))
                            Text("CodeBlog recording is currently turned off, so cards aren’t being produced.")
                                .font(.custom("Nunito", size: 13))
                                .foregroundColor(.gray.opacity(0.6))
                                .multilineTextAlignment(.center)
                                .padding(.horizontal, 16)
...
private final class ScreenshotFilmstripGenerator {
    ...
    private let queue: OperationQueue = {
        ...
        q.name = "com.codeblog.screenshotfilmstrip"
        q.maxConcurrentOperationCount = 1
        q.qualityOfService = .userInitiated
        return q
    }()

    private let syncQueue = DispatchQueue(label: "com.codeblog.screenshotfilmstrip.sync")
...
private final class ScreenshotSlideshowFrameLoader: @unchecked Sendable {
    ...
    private let decodeQueue = DispatchQueue(label: "com.codeblog.slideshow.decode", qos: .userInitiated)
