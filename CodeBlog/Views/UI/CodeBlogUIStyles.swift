...
extension View {
    /// Applies complete CodeBlog style with rounded rectangle shape
    func codeblogStyle(
    ...
    /// Applies complete CodeBlog style with circle shape
    func codeblogCircleStyle(backgroundColor: Color = .white) -> some View {
        self
            .background(backgroundColor)
            .clipShape(Circle())
    }
...
struct CodeBlogCircleButton<Content: View>: View {
    let action: () -> Void
    let size: CGSize
...
struct CodeBlogPillButton: View {
    let text: String
    let font: Font
